...
declare module "warpsocket/addon-loader" {
    ...
    function getDebugState(mode: "sockets"): Record<number, SocketDebug>;
    function getDebugState(mode: "sockets", socketId: number): SocketDebug | undefined;
    function getDebugState(mode: "workers"): Record<number, WorkerDebug>;
 ...
 * @returns For "channels" and "kv", an array of objects with detailed state information, capped at 2000 entries. For "sockets" and "workers", an object keyed by socketId/workerId with values being the debug info objects. When singleKey is provided, a single object or undefined is returned instead of an array/object (except when mode is "channels" and singleKey is a number).
 */
